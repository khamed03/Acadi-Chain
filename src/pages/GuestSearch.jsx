...
export default function GuestSearch() {
  ...
  useEffect(() => {
    API.pendingCertificates().then((data) => setPending(data || [])).catch(() => setPending([]));
  }, []);
  ...
  async function onSearch() {
    ...
    setResult(null);
    if (!query) {
      setError("Enter a CID or wallet address");
      return;
    }
    try {
      if (mode === "cid") {
        const data = await API.lookupByCid(query);
        setResult(data);
      } else {
        const data = await API.myCertificates(null, query);
        setCerts(data || []);
      }
    } catch {
      setError("Not found or invalid format");
    }
  }
  ...
  return (
    <div>
      <h1>Guest lookup</h1>
      <div>
        <button onClick={() => setMode("cid")}>By CID</button>
        <button onClick={() => setMode("wallet")}>By wallet address</button>
      </div>
      <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={mode === "cid" ? "bafy..." : "0x..."} />
      <Button onClick={onSearch}>Search</Button>
      {error && <div style={{ color: "red" }}>{error}</div>}
      {mode === "cid" && result && (
        <div>
          <p>Owner: {result.student || "N/A"}</p>
      ...
      {mode === "wallet" && (
        <ul>
          {certs.map((c) => (
            <li key={c.cid}>{c.name} – {c.degree} [{c.cid}]</li>
          ))}
        </ul>
      )}
